...
async function callChat({ apiUrl, apiKey, body, maxRetries, baseDelayMs }) {
...
function messagesForIteration({ equation, variable, context, prior, planStep }) {
  ...
  const schema = `Return ONLY valid JSON with keys:{"k":number,"goal":string,"analysis":string,"equations":Array<{"latex"?:string,"text"?:string}>,\"result_summary\":string,\"latex\"?:string,\"stop\"?:boolean,\"main_result_latex\"?:string}. In the 'equations' array, each element MUST correspond to exactly one equation statement (one per line). Do NOT put prose in 'latex'.`
...
async function tryRepairJSON({ apiUrl, apiKey, model, temperature, content }) {
  ...
  const data = await callChat({ apiUrl, apiKey, body, maxRetries: process.env.SCH_MAX_RETRIES || 3, baseDelayMs: process.env.SCH_BACKOFF_BASE_MS || 500 })
...
function validateIteration(it, last) {
  if (!it) return false
  // Require minimum equations
  const eqs = Array.isArray(it.equations) ? it.equations : []
  if (eqs.length < 6) return false
  // Require a non-empty result summary
  if (!it.result_summary || it.result_summary.length < 40) return false
  // Basic continuity: avoid identical first equation text to last
  if (last && Array.isArray(last.equations) && last.equations.length && eqs.length) {
    const a = (last.equations[0]?.latex || last.equations[0]?.text || '').trim()
    const b = (eqs[0]?.latex || eqs[0]?.text || '').trim()
    if (a && b && a === b) return false
  }
  // Look for key physics checks mentioned at least once in analysis or equations
  const blob = `${it.analysis || ''}\n${it.result_summary || ''}`.toLowerCase()
  const keywords = ['boundary', 'normalization', 'hermit', 'dimension']
  const hit = keywords.some(k => blob.includes(k))
  if (!hit) return false
  return true
}

async function reviseIteration({ apiUrl, apiKey, model, temperature, equation, variable, context, prior, planStep, badIteration }) {
  const messages = [
    { role: 'system', content: sysPreamble },
...
export async function solveSchrodingerIterative({ equation, variable = 'x', context = {}, maxIterations = 6, temperature = 0.1, detailLevel = 'exhaustive' }) {
  ...
  for (let k = 1; k <= totalLoops && !stop; k++) {
    ...
    if (!validateIteration(parsed, iterations[iterations.length - 1])) {
      ...
      const revised = await reviseIteration({ apiUrl, apiKey, model, temperature, equation, variable, context, prior: iterations, planStep, badIteration: parsed })
